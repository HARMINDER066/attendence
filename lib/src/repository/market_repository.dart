...
Future<Stream<Product>> getPopularMarkets(Address myLocation, String limit) async {
  Uri uri = Helper.getUri('api/markets');
  Map<String, dynamic> _queryParams = {};
  SharedPreferences prefs = await SharedPreferences.getInstance();
  Filter filter = Filter.fromJSON(json.decode(prefs.getString('filter') ?? '{}'));
  _queryParams['limit'] = limit;
  _queryParams['popular'] = '';
  _queryParams['user_id'] = '${userRepo.currentUser.value.id == null?"":userRepo.currentUser.value.id}';
  if (!myLocation.isUnknown()) {
    _queryParams['myLon'] = myLocation.longitude.toString();
    _queryParams['myLat'] = myLocation.latitude.toString();
    _queryParams['areaLon'] = myLocation.longitude.toString();
    _queryParams['areaLat'] = myLocation.latitude.toString();
  }
...
Future<Stream<Market>> searchMarkets(String search, Address address,  String limit) async {
  Uri uri = Helper.getUri('api/markets');
  Map<String, dynamic> _queryParams = {};
