...
Future<bool> removeCart(Cart cart) async {
  ...
  print("removeCartResponse-->${json.decode(response.body)}");
  return Helper.getBoolData(json.decode(response.body));
}

Future<Stream<CouponList>> getCouponList() async {
  User _user = userRepo.currentUser.value;
  if (_user.apiToken == null) {
    return new Stream.value(null);
  }
  final String _apiToken = 'api_token=${_user.apiToken}&';
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}coupons?${_apiToken}';
  final client = new http.Client();
  final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
  if(streamedRest != null){
    return streamedRest.stream.transform(utf8.decoder).transform(json.decoder).map((data) => Helper.getData(data)).expand((data) => (data as List)).map((data) {
