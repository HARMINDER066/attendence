...
Future<bool> sedOtp(userModel.User user) async {
  final String url = '${GlobalConfiguration().getValue('api_base_url')}send_otp';
  final client = new http.Client();
  var map = new Map<String, dynamic>();
  map["mobile"] = user.phone;
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
    body: json.encode(map),
  );
  print("sendOtp==>${json.encode(map)}");
  if (response.statusCode == 200) {
...
Future<void> userLogout() async {
  ...
  final String url = '${GlobalConfiguration().getValue('api_base_url')}logout?$_apiToken';
  final client = new http.Client();
  print("logout==>${url}");
...
Future<Address> addAddress(Address address) async {
  ...
  print("addCurrentAddress-->${url}");
  print("addCurrentAddress-->${json.encode(address.toMap())}");
